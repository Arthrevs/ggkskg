...
export function MimicPanelMap({ nodes, edges, blocks, trains, simMinutes, selectedSectorId, trackedTrainId, routeGeometries = [] }: MapProps) {
  ...
  const activeTrains = useMemo(() => {
     ...
     return active;
  }, [trains, simMinutes, nodes]);
  const activeSector = pinnedSector || hoveredSector;

  return (
    <div className="relative w-full h-full bg-rail-panel rounded-lg border border-rail-border overflow-hidden p-2 group">
      {loading && (
        <div className="absolute inset-0 z-1000 flex items-center justify-center bg-slate-950/50 backdrop-blur-sm">
          <div className="text-emerald-400 font-mono animate-pulse">Fetching Topologic Layout...</div>
        </div>
      )}
      ...
      <MapContainer 
        key={`mimic-map-${nodes[0]?.code || 'empty'}-${nodes[nodes.length-1]?.code || 'empty'}`}
      ...
      >
        ...
        <TileLayer
          url="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAC0lEQVQI12NgAAIABQABNjN9GQAAAAlwSFlzAAAWJQAAFiUBSVIk8AAAABl0RVh0U29mdHdhcmUAcGFpbnQubmV0IDQuMC4xNkREEwAAAA1JREFUGFdjYGBg+A8AAQQBAScLuN4AAAAASUVORK5CYII="
          attribution=""
        />
        ...
        {/* Render accurate track geometry from the backend instead of straight lines */}
        {routeGeometries.map((rg, i) => {
           ...
           return (
             <React.Fragment key={`mimic-link-${rg.source}-${rg.target}-${i}`}>
               <Polyline 
