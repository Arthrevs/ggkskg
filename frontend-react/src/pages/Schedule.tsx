...
import type { ScheduleMode, ScheduleEntry, Department } from '../lib/types';
...
export default function Schedule() {
  ...
  const { data: schedule, isLoading } = useSchedule('2026-W36', mode);

  const filteredEntries = useMemo(() => {
    if (!schedule) return [];
    return schedule.entries.filter(entry => {
      if (filterDept && !entry.departments.includes(filterDept as Department)) return false;
      if (filterDay && entry.blockWindow.day !== filterDay) return false;
      return true;
    });
  ...
  const timelineData = useMemo(() => {
    ...
    const sections = [...new Set(filteredEntries.map(e => e.blockWindow.section))];
    return sections.map(section => {
      const entries = filteredEntries.filter(e => e.blockWindow.section === section);
      const row: Record<string, unknown> = { section: section.split(' – ')[0] };
      entries.forEach((entry, i) => {
        const startHour = parseInt(entry.blockWindow.startTime.split(':')[0]);
        const endHour = parseInt(entry.blockWindow.endTime.split(':')[0]) || 24;
        const duration = endHour > startHour ? endHour - startHour : (24 - startHour) + endHour;
        row[`block_${i}`] = duration;
        row[`block_${i}_dept`] = entry.departments[0];
        row[`block_${i}_start`] = entry.blockWindow.startTime;
      });
      return row;
    });
  }, [filteredEntries]);
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {schedule && (
          <div className="ml-auto flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
            <span><strong className="text-slate-700 dark:text-slate-300">{schedule.stats.totalWindowsUsed}</strong> windows</span>
            <span><strong className="text-slate-700 dark:text-slate-300">{schedule.stats.totalRequestsScheduled}</strong> scheduled</span>
            <span><strong className="text-slate-700 dark:text-slate-300">{schedule.stats.unscheduledRequests}</strong> unscheduled</span>
          </div>
        )}
      </div>
...
function ScheduleTableRow({ entry }: { entry: ScheduleEntry }) {
  return (
    <tr className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
      ...
      <td className="px-4 py-3">
        <span className="text-xs text-slate-600 dark:text-slate-400">{entry.blockWindow.section}</span>
      </td>
      <td className="px-4 py-3">
        <div className="flex flex-col gap-1">
          {entry.assignedRequests.slice(0, 3).map(req => (
            <span key={req.id} className="text-xs text-slate-600 dark:text-slate-400">
              <span className="font-mono text-slate-400">{req.id}</span> {req.description.slice(0, 40)}
              {req.description.length > 40 ? '…' : ''}
            </span>
          ))}
          {entry.assignedRequests.length > 3 && (
            <span className="text-xs text-railway-600 dark:text-railway-400">
              +{entry.assignedRequests.length - 3} more
            </span>
          )}
        </div>
      </td>
